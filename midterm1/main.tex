...
\subsection*{Part c), Limiting ourselves to one-particle-one-hole excitations.}
    We will now calculate the matrix elements involving the one-particle-one-hole states. Firstly, we compute the elements including one excited state. By using the expressions for the Hamiltonian $\hat{H} = \hat{H}_0 + \hat{H}_I$ in addition to Wick's generalized theorem, we find:
